...
Providers must stream large reads and writes, paginate listings where the protocol supports it, preserve TLS/SSH verification, and avoid advertising unsupported features. S3, SFTP, WebDAV, FTP/FTPS, and SMB are implemented provider slices. FTP directory listings are complete protocol responses rather than cursor-paginated results. Additional cloud services are Planned.
