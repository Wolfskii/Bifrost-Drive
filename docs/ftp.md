...
The optional start path scopes all remote operations below that FTP directory. It defaults to `/` so a new connection mounts the FTP root instead of the account working directory. It may be relative to the account's FTP root or an absolute FTP path, but it cannot contain `..` parent traversal.
