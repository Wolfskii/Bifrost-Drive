...
Bifrost caches resolved Google Drive directory IDs for the lifetime of the connection, avoiding repeated API walks for every file operation in deep folders. Directory listings seed the cache for every child folder, so navigating into a listed folder needs no extra lookup. Listed binary file IDs are reused for 30 seconds so consecutive range reads skip the name query; a stale ID that returns 404 is dropped and re-resolved. Workspace documents and all writes always resolve fresh metadata. Mutations invalidate only the affected path and its subtree. New files are created with one multipart upload request. Rate-limit (`403 rateLimitExceeded`/`userRateLimitExceeded`, `429`) and `5xx` responses on replayable requests are retried with capped exponential backoff that honors `Retry-After`; streamed uploads are sent once and surface failures to the transfer queue. My Drive searches use the narrower user corpus; shared drives retain drive-scoped queries.

Mounted connections poll the Drive Changes API every 15 seconds, like rclone's change notifications. Bifrost keeps a file-ID-to-path index from listings, maps each change to its previous and current virtual paths, and invalidates only those paths; changes outside listed folders are ignored because nothing about them is cached. More than 10,000 changes in one poll clear all caches instead. While polling succeeds, the drive mount caches listings for five minutes.
