...
macOS File Provider, Linux FUSE, Windows VM acceptance, protected release-secret provisioning, and future provider expansion remain Planned until implemented and tested. The Windows Credential Manager and CFAPI adapters, durable cache/transfer integration, scheduled conflict-safe sync orchestration, activity history, native notifications, updater configuration, S3 provider, verified WebDAV/SFTP operations, SFTP public-key flow, and verified WebDAV advanced operations are implemented and tested. Capabilities may be exposed only where the provider implementation and tests verify them. Do not expose unavailable capabilities in the UI.
