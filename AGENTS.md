...
macOS File Provider, Linux FUSE, background sync scheduling, Windows VM acceptance, and release packaging remain Planned until implemented and tested. The Windows Credential Manager and CFAPI adapters, durable cache/transfer integration, conflict-safe sync orchestration, S3 provider, verified WebDAV/SFTP operations, SFTP public-key flow, and verified WebDAV advanced operations are implemented and tested. Capabilities may be exposed only where the provider implementation and tests verify them. Do not expose unavailable capabilities in the UI.
